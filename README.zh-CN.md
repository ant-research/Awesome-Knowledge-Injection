...
| 主干论文 | 5+ | 直接围绕 VLM / MLLM 知识注入且强调通用能力不遗忘 |
...
除 `Distill` 外，下表均按时间倒序（newest first）排列。`Distill` 先分成 `直接相关 VLM / MLLM 工作` 与 `LLM 上游启发` 两组，各组内部按时间倒序排列。`GitHub / Stars` 只记录公开官方仓库，星标为 `2026-04-19` 可验证快照；`主要数据集 / 基准` 只列最核心实验入口。
